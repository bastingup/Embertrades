...
const cryptoAssets =  ["BNB", "ETH", "ADA"]
//const cryptoAssets = ["DOT"]
// Favs: DOT, MANA, LINK, AAVE
var selectedAssets = ["BNB", "ETH", "ADA"]
                        ...
                        openOrders.forEach(function(openOrder) {
                         ...
                         //   console.log("ATR", signal.SIGNAL_ATR, "POFIT AT", openOrder.profitableAt, "CURRENT PRICE", signal.price, "FEE", fee)
                            ...
                            if (signal.price < stopLoss) {
                                ...
                                console.log("\u001B[31mSTOPLOSS \u001B[37m- Closed position for:", openOrder.asset, "with", openOrder.price, "at", signal.price, "totaled at:",parseFloat(((signal.price / openOrder.price) * 100).toFixed(2)), "%.")
...
// ------------------------- BUY SPOT ORDER --------------------------------------------
            ...
            console.log("Return on investment:", parseFloat(((finalWalletValue / config.availableBacktestBUSD * 100) - 100).toFixed(1)), "% after just about", parseInt(Math.floor(numberOfDays)), "hours of trading.")
